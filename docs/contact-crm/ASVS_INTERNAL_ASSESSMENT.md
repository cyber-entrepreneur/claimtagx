...
| **Session management** | **PASS (local)** | **BLOCKED** | HttpOnly + `SameSite=None` + `secure` in production; opaque hashed tokens; **session revocation** via first-party auth tables on logout | Live revocation soak **NOT RUN** |
