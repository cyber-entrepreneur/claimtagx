...
**Schema head:** `0019_crm_staff_routing_attributes.sql`
...
| --- | --- | --- | --- | --- |
| **Authentication (V2)** | **PASS (local)** | **BLOCKED** | Clerk middleware + staff session HMAC (`requirePlatformAdmin.ts`); legacy access-key login gated by `authFlags.ts`; **constant-time** session sig verify (`timingSafeEqual` in `decodeStaffSession`); **constant-time** access-key compare on login (`platformContact.ts` auth/login) | Live Clerk tenant matrix incomplete; shared access-key path is legacy/dev-gated (`authFlags`) |
| **Authorization / RBAC (V4)** | **PASS (local)** | **BLOCKED** | `rbac.ts` permission catalog; `requirePermission` on platform mutations; `routePermissions.source.test.ts`; `rbacHttp.pg.test.ts`; `rbac.matrix.test.ts`; `multiRoleHttp.pg.test.ts` | Real Clerk roles **NOT RUN** |
...
| **Session management** | **PASS (local)** | **BLOCKED** | HttpOnly + `SameSite=lax` + `secure` in production; dedicated `PLATFORM_STAFF_SESSION_SECRET`; **session revocation** via `crm_session_revocations` on logout | Live IdP revocation soak **BLOCKED** without Clerk |
...
| **Production debug** | **PASS (local)** | **NOT RUN** | Pretty logs off in production; Clerk key required; Graph/attachment readiness gates; `assertProductionSecurity` refuses test auth, simulators, embed worker | Confirm deploy env does not set forbidden flags |
...
3. Residual blockers: live Clerk, live Graph, live object storage, formal ASVS review, qualified WCAG, Arabic legal review.
