...
| `migrationApply.deferred.test.ts` | skipped without `CRM_ALLOW_MIGRATE` | **Remediated** - non-destructive assert schema head = **0023** |
...
Local isolated only (PG `55432`, API `18080`, Vite `5173`). Not production / first-party auth staging / Graph live.
