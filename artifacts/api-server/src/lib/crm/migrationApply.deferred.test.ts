...
describe("CRM SQL migrations (deferred live apply)", () => {
  it("has ordered SQL files ready for psql through schema head 0020", () => {
    ...
    assert.equal(files.filter((f) => /^\d{4}_/.test(f)).length >= 20, true);
    for (const name of files) {
      assert.ok(existsSync(join(drizzleDir, name)), name);
    }
  });

  it("isolated verify DB already reports schema head >= 0020 (non-destructive)", async () => {
    ...
    try {
      ...
      assert.match(head, /^0020_|^00[3-9]\d_/);
      assert.ok(
        head >= "0020_crm_omnichannel_inbox.sql",
        `expected schema head >= 0020, got ${head}`,
      );
    } finally {
      await client.end();
    }
